...
**实体：**
1. **Actor（演员）**
   - actor_id: 演员的唯⼀标识符（ID），整数类型。
   - actor_name: 演员的名字。

2. **Director（导演）**
   - director_id: 导演的唯⼀标识符（ID），整数类型。
   - director_name: 导演的名字。

3. **Movie（电影）**
   - movie_id: 电影的唯⼀标识符（ID）。
   - movie_title: 电影的标题。
...
```Cypher
// 在Actor节点上创建约束
CREATE CONSTRAINT FOR(a:Actor)REQUIRE a.actor id IS UNIQUE;
//导入Actor数据
LOAD CSV WITH HEADERS FROM 'file:///actor.csv'AS rowCREATE(:Actor {actor id: toInteger(row.actor id), actor _name:row.actor name});

// 在Director节点上创建约束
CREATE CONSTRAINT FOR(d:Director)REOUIRE d.director id IS UNIOUE;
//导入Director数据
LOAD CSV WITH HEADERS FROM 'file:///director.csv' AS row
CREATE(:Director {director_id: toInteger(row.director_id), director name:row.director name});

// 在Movie节点上创建约束
CREATE CONSTRAINT FOR(m:Movie)REQUIRE m.movie id IS UNIOUE;
//导入Movie数据
...
MATCH (a:Director {director_id: toInteger(row.director_id)})
WITH a, row
MATCH (m:Movie {movie_id: row.movie_id})
CREATE (a)-[:DIRECT]->(m);
```
...
- 在数据库中只存储三类节点(演员、导演、电影)以及两种关系(导演与电影间的执导关系和演员。与电影之间的参演关系)，便于查询时快速进行计算
